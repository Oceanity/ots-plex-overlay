...
// Keep track of current art queue to prevent duplicate processes racing
let coverQueue = [];
...
class PlexReader {
    constructor(options = {}) {
        ...
        this.Params.cacheCovers = options.cacheCovers || true;
    ...
    Kill() {
        Con.Log("Killing Plex Reader Instance");
        this.IsAlive = false;
    }
}
...
function fetchThumb(parser, thumb, data, callback) {
    ...
    if (thumb && data.prevThumb !== thumb) {
        ...
        // If in queue, just return server date as it is still being worked on
        if (coverQueue.indexOf(thumb) !== -1) {
        ...
        // Else, queue to avoid issues
        coverQueue.push(thumb);

        // Handle URL
        request.head(url, (err, res, body) => {
            if (err) throw err;
            else if (res.statusCode === 200) {
                let fullpath = `${cachePath}\\${id}${ext}`,
                    imageUri = `${coversUri}/${id}${ext}`;

                // Cached cover method
                if (cacheCovers) {

                    // If cached cover exists, return and remove thumb from queue
                    if (fs.existsSync(fullpath)) {
                        callback(imageUri);
                        updateQueue(thumb);
                    }

                    // Else get and encode image from server
                    else {
                        ...
                        file.on("close", () => {
                            // Return server URL first time to avoid race conditions (seriously, what on earth fs)
                            base64Encode(url, (data) => {
                                callback(data);
                                updateQueue(thumb);
                            });
                ...
                // Non cached version (can cause lag))
                else {
                    base64Encode(url, (data) => {
                        callback(data);
                        updateQueue(thumb);
...
function updateQueue(thumb) {
    coverQueue = coverQueue.splice(coverQueue.indexOf(thumb), 1);
}
