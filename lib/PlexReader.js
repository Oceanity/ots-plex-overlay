...
function encodeThumb(parser, thumb, data, callback) {
    ...
    if (thumb && data.prevThumb !== thumb) {
        let id = thumb.split("/").pop();
        // If cached image exists, use instead of pulling from server
        if (fs.existsSync(dir + id)) {
            Con.Log("Loading album cover from cache.");
            fs.readFile(dir + id, "utf8", (err, data) => {
                if (err) throw err;
                callback(data);
            });
        }
        else {
            Con.Log(`Beginning Base64 encode of ${thumb}...`);
            let url = `http${
                encrypted ? "s" : ""
            ...
            request.get({
                ...
                (error, resp, body) => {
                    if (!error && resp.statusCode == 200) {
                        if (callback && typeof callback === "function") {
                            fs.writeFile(dir + id, `data:${resp.headers["content-type"]};base64,${Buffer.from(body).toString("base64")}`, (err) => {
                                if (err) throw error;
                                Con.Log(`Wrote encoded cover to ${dir + id}`)
                                callback(
                                    `data:${
